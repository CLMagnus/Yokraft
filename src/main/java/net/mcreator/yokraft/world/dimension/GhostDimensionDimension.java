...
package net.mcreator.yokraft.world.dimension;
...
import net.mcreator.yokraft.procedures.GhostDimensionPlayerEntersDimensionProcedure;
import net.mcreator.yokraft.item.GhostDimensionItem;
...
public class GhostDimensionDimension extends YokraftElements.ModElement {
	@ObjectHolder("yokraft:ghostdimension")
	public static final ModDimension dimension = null;
	@ObjectHolder("yokraft:ghostdimension_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public GhostDimensionDimension(YokraftElements instance) {
		super(instance, 39);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("ghostdimension"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("yokraft:ghostdimension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("yokraft:ghostdimension"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("yokraft:ghostdimension"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("yokraft:ghostbiome")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new GhostDimensionItem().setRegistryName("ghostdimension"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("ghostdimension_portal");
		}
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos)) || !(this.world.getBlockState(blockpos.down())
							.getBlock() == AtamantineOreBlockBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == AtamantineOreBlockBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == AtamantineOreBlockBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos,
									flag
											? AtamantineOreBlockBlock.block.getDefaultState().getBlock().getDefaultState()
											: Blocks.AIR.getDefaultState());
						}
					}
				}
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos,
								AtamantineOreBlockBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		if (event.getTo() == type) {
			{
				...
				GhostDimensionPlayerEntersDimensionProcedure.executeProcedure($_dependencies);
			}
		}
	}
