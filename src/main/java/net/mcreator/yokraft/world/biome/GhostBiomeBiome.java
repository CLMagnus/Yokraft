...
package net.mcreator.yokraft.world.biome;
...
import net.mcreator.yokraft.block.DeadleavesBlock;
...
public class GhostBiomeBiome extends YokraftElements.ModElement {
	@ObjectHolder("yokraft:ghostbiome")
	public static final CustomBiome biome = null;
	public GhostBiomeBiome(YokraftElements instance) {
		super(instance, 40);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.2f).temperature(0f).precipitation(Biome.RainType.NONE)
					.category(Biome.Category.NETHER).waterColor(-10066330).waterFogColor(-10066330).parent("the_end")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.MYCELIUM.getDefaultState(),
							Blocks.COBBLESTONE.getDefaultState(), Blocks.COBBLESTONE.getDefaultState())));
			setRegistryName("ghostbiome");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.MYCELIUM.getDefaultState().getBlock() || ground == Blocks.COBBLESTONE.getDefaultState().getBlock())
							&& (ground2 == Blocks.MYCELIUM.getDefaultState().getBlock()
									|| ground2 == Blocks.COBBLESTONE.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), Blocks.COBBLESTONE.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == DeadleavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, DeadleavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												Blocks.BLUE_ORCHID.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.DARK_OAK_LOG.getDefaultState().getBlock()
					|| blockType == DeadleavesBlock.block.getDefaultState().getBlock() || blockType == Blocks.MYCELIUM.getDefaultState().getBlock()
					|| blockType == Blocks.COBBLESTONE.getDefaultState().getBlock();
		}
